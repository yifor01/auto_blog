...
若作者來自頂尖 AI 研究機構（如 Google DeepMind、OpenAI、Anthropic、Meta AI、Microsoft Research、NVIDIA、DeepSeek 等），應在標題或開頭段落明確標註機構名稱，例如「【Anthropic 最新研究】」或「Google DeepMind 團隊提出…」。這是吸引讀者點擊的重要訊號。

### 破折號使用限制

整篇貼文中破折號（——）最多使用 2-3 個。過度使用破折號會讓文章讀起來像 AI 生成的，降低可信度。優先使用逗號、句號、分號等標點來組織句子。
...
def generate_blog_post(item: ScoredItem) -> GeneratedContent:
    ...
    user_msg = f"""請根據以下資訊，撰寫一篇 Facebook 科技部落格貼文。以下是目前可獲得的所有資訊，請基於這些內容撰寫，不要臆測或捏造未提及的細節：
