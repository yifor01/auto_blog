...
VARIANT_CASES = [
    ...
    ("超額加註", "尚頎資本、蔚來資本等都在本輪超額加註。",
     "尚頎資本、蔚來資本等都在本輪超額加注。"),
    # ⑥ 2026-08-02 `opencc-candidates` 第一輪審查產出。與上面幾批的差別是候選
    # 來自工具（歧義組用 OpenCC 自己的字典建）而非人工逐句判讀，且每個錨定 key
    # 都對全語料 91 天 12830 筆實跑過（命中 100 處、誤傷 0 處）。
    # 文字一律取自真實語料的實際上下文。
    ("伙伴", "而是扮演著合作伙伴的角色。", "而是扮演著合作夥伴的角色。"),
    ("髮起", "李飛飛學生髮起國際具身人類資料標準",
     "李飛飛學生發起國際具身人類資料標準"),
    ("髮掘", "核心原則：一是引導學生髮掘志趣", "核心原則：一是引導學生發掘志趣"),
    ("髮明", "大概只相當於電燈泡剛剛被髮明出來", "大概只相當於電燈泡剛剛被發明出來"),
    ("黃髮了", "幾天前，老黃髮了人生第一條推", "幾天前，老黃發了人生第一條推"),
    # 這條的期望值同時吃到 `髮起`——語料裡兩個錯字就是連在一起的
    ("鉅頭", "360聯手信創鉅頭髮起“磐石之盾”安全協作計劃",
     "360聯手信創巨頭發起“磐石之盾”安全協作計劃"),
    ("鉅作", "提示詞爆火！24小時復刻3A鉅作", "提示詞爆火！24小時復刻3A巨作"),
    ("鉅額", "計入每年訓練下一代模型的鉅額開支", "計入每年訓練下一代模型的巨額開支"),
    ("是隻靠", "分析陌生情況，而不是隻靠見過的案例作出反應",
     "分析陌生情況，而不是只靠見過的案例作出反應"),
    ("是隻能", "學會和混亂共存。這是隻能靠自己悟出來的能力",
     "學會和混亂共存。這是只能靠自己悟出來的能力"),
    ("是隻要", "我最感激一件事。那就是隻要你開口求助",
     "我最感激一件事。那就是只要你開口求助"),
    ("是隻把", "而不是隻把原文當作臨時上下文", "而不是只把原文當作臨時上下文"),
    ("雲百鍊", "兩款模型已在阿里雲百鍊開放呼叫。", "兩款模型已在阿里雲百煉開放呼叫。"),
    ("線百鍊", "適配Qwen3.8，上線百鍊提供推理服務",
     "適配Qwen3.8，上線百煉提供推理服務"),
    ("百鍊M", "打造“Agent工廠”，百鍊MCP服務上線",
     "打造“Agent工廠”，百煉MCP服務上線"),
    ("百鍊或", "MiniMax、智譜、百鍊或自定義端點", "MiniMax、智譜、百煉或自定義端點"),
    ("公有云", "其中公有云服務毛利率為 -119.0%", "其中公有雲服務毛利率為 -119.0%"),
    ("阿里云云", "阿里云云監控CMS2.0將此前獨立的功能整合",
     "阿里雲雲監控CMS2.0將此前獨立的功能整合"),
    ("多采用", "人形機器人運控模型大多采用淺層MLP網絡",
     "人形機器人運控模型大多採用淺層MLP網絡"),
    ("多采一", "它不是單純多采一點影片", "它不是單純多採一點影片"),
    ("下采集", "並不只是“省下采集資料的錢”", "並不只是“省下採集資料的錢”"),
    ("和麵向", "MoWorld 2.0和麵向更高推理效率的影片生成基模",
     "MoWorld 2.0和面向更高推理效率的影片生成基模"),
    ("和麵對", "人與人之間的聊天、討論和麵對面交流",
     "人與人之間的聊天、討論和面對面交流"),
    ("儀錶板", "跟進行動項，甚至生成報告和儀錶板", "跟進行動項，甚至生成報告和儀表板"),
    ("對錶現", "策略在模擬中的相對錶現，能否預測它回到真機後的表現",
     "策略在模擬中的相對表現，能否預測它回到真機後的表現"),
    ("綵線", "白圈是路口，綵線是道路，A到J是十個頂點。",
     "白圈是路口，彩線是道路，A到J是十個頂點。"),
    ("全綵", "在高階全綵 AR 品類，雷鳥 X3 Pro", "在高階全彩 AR 品類，雷鳥 X3 Pro"),
]


# ══════════════════════════════════════════════════════════
# `_LOCATIVE_LI_RE`：方位的「里」→「裡」
# ══════════════════════════════════════════════════════════
#
# 為什麼不進 `_VARIANT_FIXES`：`str.replace` 表達不了「後面不是『程』」這個條件，
# 與 `_YINSHU_RE`（`吸引數` 不可改成 `吸參數`）同型。

@pytest.mark.parametrize("wrong,right", [
    ("比如你在頻道里對一個戰士說", "比如你在頻道裡對一個戰士說"),
    ("核技術的自動駕駛賽道里，西井科技", "核技術的自動駕駛賽道裡，西井科技"),
    ("使用者可直接在文本里嵌入", "使用者可直接在文本裡嵌入"),
    ("電路里常指電壓或電流", "電路裡常指電壓或電流"),
    ("比如把網格里的一堆方形圖片縮小", "比如把網格裡的一堆方形圖片縮小"),
    ("這整個過程里人類做的最大一次干預", "這整個過程裡人類做的最大一次干預"),
    ("改動只在隔離副本里進行", "改動只在隔離副本裡進行"),
    ("散在無數份檔案和渠道里", "散在無數份檔案和渠道裡"),
    ("一條叫殘差流的資訊通道里寫入", "一條叫殘差流的資訊通道裡寫入"),
    ("它會去不同應用里拉取資料", "它會去不同應用裡拉取資料"),
    ("憑什麼在一年多里翻了十幾倍", "憑什麼在一年多裡翻了十幾倍"),
])
def test_locative_li_is_fixed(wrong, right):
...
@pytest.mark.parametrize("text", [
    # 這些「里」是正確用字，通用的 `里→裡` 會把它們全部改壞
    "最後讓強化學習走完最後一公里",
    "這一規模在行業內具有里程碑意義",
    "阿里巴巴集團旗下高德正式發佈",
    "Google Cloud 的 CEO 托馬斯·庫里安",
    "報告廳門外的拉里·古斯",
    "千里科技與吉利汽車",
    "生成公里級3D城市場景",
    # `(?!程)` 專門擋的：容器名詞後面接「里程」，那是正確用字
    "這條賽道里程碑式的突破",
    "隧道里程統計",
    "網格里程資料",
])
def test_locative_li_leaves_correct_text_alone(text):
    from src.utils import _apply_variant_fixes
