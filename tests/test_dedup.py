...
class TestArxivVersionSuffixDedup:
    """arxiv collector 存的 id 帶版本後綴（2607.08772v1），hf_papers /
    semantic_scholar 不帶。ContentItem.dedup_key() 比對前會去掉版本後綴，
    get_seen_urls() 也必須做同樣正規化，否則兩側算出的 key 永遠對不上，
    arXiv 論文的跨日去重等同全程失效。
    """

    def test_seen_set_strips_arxiv_version_suffix(self, tmp_path):
        raw_dir = tmp_path / "raw"
        raw_dir.mkdir()
        items = [{"url": "https://arxiv.org/abs/2607.03803v1",
                  "raw_metadata": {"arxiv_id": "2607.03803v1"}}]
        (raw_dir / "2026-07-10.json").write_text(json.dumps(items))

        with patch("src.utils.RAW_DIR", raw_dir):
            seen = get_seen_urls(exclude_date=date(2026, 7, 11), lookback_days=7)

        assert "arxiv:2607.03803" in seen, "版本後綴未去除，跨日去重會漏掉整組 arXiv 論文"

    def test_versioned_history_matches_unversioned_dedup_key(self, tmp_path):
        """昨天由 arxiv collector 收到 v1，今天 hf_papers 收到同一篇（無版本）→ 應命中。"""
        raw_dir = tmp_path / "raw"
        raw_dir.mkdir()
        yesterday = [{"url": "https://arxiv.org/abs/2607.03803v1",
                      "raw_metadata": {"arxiv_id": "2607.03803v1"}}]
        (raw_dir / "2026-07-10.json").write_text(json.dumps(yesterday))
        ...
        today = ContentItem(
            ...
            raw_metadata={"arxiv_id": "2607.03803"},
        )

        with patch("src.utils.RAW_DIR", raw_dir):
            seen = get_seen_urls(exclude_date=date(2026, 7, 11), lookback_days=7)

        assert today.dedup_key() in seen, "同一篇論文跨來源、跨日未被去重"
