...
def test_priority3_fetch_when_short(monkeypatch):
    """abstract < 200 字時補抓文章 URL。"""
    from src.collectors.rss_collector import RSSCollector

    entry = _make_entry(summary="too short")
    long_text = "word " * 100  # > 200 字
    ...
    result = RSSCollector._extract_abstract(entry, "http://example.com", mock_client, min_len=200)
...
def test_no_fetch_when_long_enough():
    """abstract >= 200 字時不觸發補抓。"""
    from src.collectors.rss_collector import RSSCollector

    long_content = "<p>" + "word " * 60 + "</p>"  # 60 * 5 = 300 chars
    entry = _make_entry(content_val=long_content)
    client = MagicMock()
    result = RSSCollector._extract_abstract(entry, "http://example.com", client, min_len=200)
