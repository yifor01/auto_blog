...
TL;DR：透過全參數微調、知識蒸餾與權重插值，ZooClaw‑FashionSigLIP2 在新建時尚檢索基準上超越既有方法，並緩解資料集結構偏見。

🧩 針對時尚檢索的 VL 模型新方向  
現有的視覺‑語言（Vision‑Language, VL）模型多以通用影像資料訓練，直接套用於時尚商品檢索時常會受到資料集結構偏見（例如顏色或類別分佈不均）的限制。作者提出一套「全參數微調 + 知識蒸餾 + 權重插值」的訓練流程，專門針對時尚領域進行調整，旨在提升檢索的穩健性與精準度。

🧩 方法與架構概述  
1. **全參數微調（Full Fine‑tuning）**：在時尚影像‑文字配對資料上，對基礎 VL 模型的所有參數進行微調，讓模型學習到時尚特有的視覺語意對應。  
...
🎯 實務啟示  
- 若你的團隊正在建置時尚商品搜尋或推薦系統，可考慮在現有 VL 模型上加入全參數微調，並同步使用知識蒸餾與權重插值，以提升模型對時尚細節的感知與檢索穩定性。  
