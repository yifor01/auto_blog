...
- 團隊發現「模型」不只是純粹的語言生成器，而是與整個堆疊深度耦合的供應商專屬行為，包括參數填充方式、工具呼叫語法等。這些隱藏的差異在遷移過程中需要逐一驗證與調整。
...
2. **首次 GPT‑5.6 評估**  
   - 直接以 GPT‑5.6 Sol 替換模型，跑相同任務。首次測試出現了「真實失敗模式」：模型在工具參數填寫上與 Opus 不同，導致部分程式碼生成錯誤。  

3. **錯誤逐一定位**  
   - 團隊根據每一次失敗的回報，調整 Vercel AI SDK 中的參數對映與提示詞（prompt），確保 GPT‑5.6 能正確填入工具參數。  
...
- **工具參數相依**：不同供應商的 LLM 在填寫工具參數時的預設格式不同，需要在 SDK 層面做適配。  
