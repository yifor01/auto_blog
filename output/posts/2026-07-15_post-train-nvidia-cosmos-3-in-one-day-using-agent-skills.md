...
開發者想把視覺推理模型套用到生產級影片任務時，往往耗費數天在資料格式調整、容器設定與超參數搜尋上，卻還不確定後訓練是否真的有用。如果有一套 agent 能幾乎自動搞定呢？

🤔 **後訓練視覺模型的前置作業吃掉數天**
開發者適應視覺推理模型到生產影片任務時，常迷失在資料格式化、容器建置、訓練指令碼、基線評估與超參數掃描中，才能知道後訓練有無提升準確率。

🧩 **Cosmos 3 混合 Transformer 架構搭配 TAO Agent Skills**
NVIDIA Cosmos 3 是開放實體 AI 世界基礎模型，採用 mixture-of-transformers 架構統一多模態輸入，並分離推理與生成路徑。TAO agent skills 將後訓練工作流程封裝起來，透過 TAO AutoML 自動化資料集處理、基線評估、LoRA 配置與超參數最佳化，實現極少人工介入的領域適應。
