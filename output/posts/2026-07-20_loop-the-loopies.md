...
長期以來，looped Transformer 面臨一個根本難題：若預訓練運算增加 N 倍，通常把參數量放大 N 倍的效果，會比把同一個模型迴圈 N 次來得好。這篇論文提出的 Loopie 正是為了解決這個痛點。

🤔 **迴圈 Transformer 的老問題：迴圈不如放大參數**

過去研究指出，給定 N 倍預訓練運算，直接將模型參數數量乘上 N，往往優於把模型 loop N 次。這讓 looped Transformer 在擴展效率上一直處於劣勢。
...
README／摘要指出，Loopie 針對前述「迴圈不如放大參數」的挑戰提出對應設計，但未詳述具體架構細節。
...
對追求運算效率的團隊而言，Loopie 顯示 looped Transformer 搭配 MoE 有可能在相同預訓練成本下，追上甚至超越傳統放大參數的路線；其在奧林匹亞級推理的表現，也提示迴圈架構未必犧牲推理深度，值得後續追蹤開放權重與實測。
