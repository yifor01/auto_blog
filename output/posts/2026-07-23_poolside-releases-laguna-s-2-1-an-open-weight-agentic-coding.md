...
🧩 **118B MoE 架構：以輕量化的參數運算實現高效能**

Laguna S 2.1 採用 Mixture-of-Experts (MoE) 架構，雖然總參數達 118B，但展現了極高的稀疏性優勢：
- 參數運算：每個 token 僅啟用約 8B 參數（約佔總參數的 6.8%）。
- 記憶體需求：所有 118B 參數均需常駐於記憶體中，但單步運算負擔極低。
...
- **Terminal-Bench 2.1**：在啟用「思考模式（Thinking mode）」下取得 70.2% 的分數，成為 Poolside 彙整排行榜中，在公開參數規模模型中的第一名。
- **DeepSWE v1.1 資料對比**：Laguna S 2.1 取得 40.4% 的成績，而 DeepSeek-V4-Pro-Max 僅為 9.0%，且後者的主動參數規模約為前者的六倍。
...
對於需要處理複雜編碼任務的工程師而言，Laguna S 2.1 證明了透過 MoE 架構與長達 1M tokens 的上下文視窗（Context Window），中型規模模型也能在高度複雜的編碼任務中，展現出足以媲美超大型模型的能力。
