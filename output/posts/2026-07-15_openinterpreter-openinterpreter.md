...
從工程實踐看，Open Interpreter 強調在 macOS、Linux、Windows 原生沙箱中執行命令，並將配置留在本地，符合本地優先（local-first）需求。它同時支援 exec、MCP、skills、hooks、permissions、AGENTS.md 等機制，便於整合進現有工作流。對於不想繫結高價 API、希望用低成本模型試驗 coding agent 的團隊，這個 Rust 重寫的 fork 值得一試。
