...
NVIDIA Ising Decoding 管線採用針對三角色碼設計的 3D 卷積神經網路（CNN）預解碼器，實現可擴展、低延遲且精準的即時解碼。該設計可針對特定量子處理器單元（QPU）架構與噪聲剖面調整，並開放 Ising 模型系列，包含權重、訓練配方、基於 cuQuantum 與 cuStabilizer 的合成資料生成工具，以及完整訓練流程。
