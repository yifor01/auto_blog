...
當面對包含複雜表格、段落與腳註的密集型檔案時，傳統 OCR 往往難以精準捕捉細節。這篇教學展示了如何利用 Baidu 的 Unlimited-OCR 模型，建立一個能處理高解析度影像與多頁 PDF 的完整工作流。
...
2. **模型載入**：從 Hugging Face 載入 3B 參數的 tokenizer 與模型，並將其切換至評估模式（evaluation mode）後移至 GPU。
...
為了處理更完整的數位檔案，流程中整合了 `PyMuPDF` 進行 PDF 解析，並透過 `infer_multi()` 函式將單頁 OCR 的能力擴展至多頁 PDF。這套流程能夠在處理過程中保持長文本生成的設定，確保跨頁內容、表格與段落的結構化輸出具備可重現性（reproducible）。
