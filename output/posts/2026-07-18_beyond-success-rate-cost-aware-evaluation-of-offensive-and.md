...
TL;DR：以固定推論成本比較紅藍隊安全代理，揭示兩者不同的擴展特性與實用門檻。
...
- 攻擊型 CTF 表現隨 test-time compute 增加而提升；擴展後的開放權重（open-weight）模型可逼近前沿專有系統，且保持成本競爭力。
- 防禦型 SOC 調查則不以此方式擴展：成功更依賴紀律化的工具使用、telemetry 導航與選擇性 enrichment，而非單純堆疊推理預算。
