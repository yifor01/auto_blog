...
TL;DR：將預訓練 diffusion transformer 的 token 直接對映為稠密任務輸出，可在少量額外參數下達到 SOTA。
...
- 這樣的對映僅需極少的額外參數，因為大部分特徵抽取已由原始擴散模型完成。

📊 **實驗結果：少量參數即能領跑**  
- 在多項標準稠密預測基準（如深度估計、語意分割）上，該方法以「最小化新增參數」的設定取得了當前最佳表現。  
...
2. **參數經濟**：只需在 token 後加上簡單投影層，即可適配各類稠密任務，避免從頭訓練大型編碼器。  
...
- 在資源受限的邊緣裝置上，這種「少參數」的稠密預測方案特別適合，因為大部分計算仍由已最佳化的 transformer 完成。
