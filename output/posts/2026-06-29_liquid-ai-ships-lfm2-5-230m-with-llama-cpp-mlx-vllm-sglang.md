...
TL;DR：230M 參數混合架構模型，主打行動裝置與機器人上的資料擷取與工具呼叫。

當大模型追求參數規模時，Liquid AI 選擇往反方向走。他們推出了至今最小的模型 LFM2.5-230M，目標非常明確：不追求通用推理，而是要讓手機、機器人與自動化裝置能高效執行 agentic 任務。
...
1. **預訓練**：使用 19 兆 tokens 進行訓練，其中包含一個 32K 的上下文擴展階段。
2. **後訓練三部曲**：
