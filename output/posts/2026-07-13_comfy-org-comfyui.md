...
🎣 **為什麼視覺創作者會在意「每個模型、每個引數、每個輸出」？**  
在傳統的 AI 生成工具中，使用者往往只能透過預設的 UI 或簡單的指令列引數調整，難以微調模型細節或自訂複雜流程。ComfyUI 把這些都搬到可拖拉的節點圖上，讓不寫程式的創作者也能像搭積木般設計完整的生成管線。

🧩 **核心設計：節點圖 + 模組化 API**  
- **節點圖介面**：每個模型、資料前處理、引數設定與後處理都以節點呈現，使用者可自由連線形成有向圖，完整描述資料流向。  
