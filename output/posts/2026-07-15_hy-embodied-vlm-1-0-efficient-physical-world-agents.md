...
🎣 **3B 啟用引數竟能逼近 32B 具身模型**
打造能真正在物理世界行動的 AI，不只是看懂影像就行。新模型 Hy-Embodied-VLM-1.0 僅用 3B 啟用引數，效能卻逼近前代 32B 啟用引數的規格，為具身代理帶來新可能。
...
🎯 **低延遲部署的具身 AI 新選擇**
對工程師而言，此模型證明瞭「小啟用引數 + 精心設計的資料與架構」能在物理世界任務逼近大模型，適合邊緣或延遲敏感場景。若實際專案需要具身推理與互動，可持續關注其後續釋出的部署資源。
