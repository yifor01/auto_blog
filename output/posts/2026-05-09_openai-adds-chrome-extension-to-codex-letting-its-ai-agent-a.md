...
📌 【OpenAI 新功能】Codex 加入 Chrome 擴展，可直接使用已登入的瀏覽器存取 LinkedIn、Salesforce、Gmail 與內部工具  
...
🧪 **擴展如何運作**  
OpenAI 現在提供了一個適用於 Mac 與 PC 的 Codex Chrome 擴展。當 Codex 需要執行必須使用使用者已登入的瀏覽器狀態時，它會呼叫此擴展，讓 Chrome 成為其操作環境。對於不需要登入的任務——如本地開發伺服器、檔案預覽或公開網頁——Codex 仍會使用內建的沙盒瀏覽器，以避免干擾使用者的 Chrome 設定。  
...
⚠️ **目前的限制與注意事項**  
- 該擴展僅支援 Chrome（Mac 與 PC 版），其他瀏覽器暫未提及支援計畫。  
